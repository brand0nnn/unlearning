...
finetuned checkpoint. This lets you confirm the model actually MEMORIZED before
running the (expensive) unlearn/evaluate pipeline: a healthy finetune should jump
from ~0.36 (base) to ~0.98 (finetuned), just like the paper.

    python scripts/tofu_check_rouge.py               # base vs tofu_learn_full_full
    python scripts/tofu_check_rouge.py --n 100       # larger sample
    python scripts/tofu_check_rouge.py --checkpoint experiments/tofu_learn_full_full
"""
import argparse
import sys
