...
checkpoint (to experiments/) and a metrics record (to {_HERE}/out/, NOT the
main results/ folder).
...
that ALL locations can reach with retain intact, and rerun with that --target.
    ...
    python studies/lora_locality/unlearn.py --learned experiments/tofu_learn_full_full \
...
def main():
    ...
    out_dir = ensure_dir(f"{_HERE}/out/{args.scheme}/unlearn")
