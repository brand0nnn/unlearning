...
{_HERE}/out/<scheme>/spectral/*.json.

    python studies/lora_locality/plot_spectral.py --scheme fixedbudget
"""
import argparse
import json
import sys
...
def main():
    ...
    sdir = Path(f"{_HERE}/out/{args.scheme}/spectral")
    files = sorted(sdir.glob("*.json"))
    if not files:
        logger.warning("no spectral JSON in %s — run studies/lora_locality/spectral.py first", sdir)
        return
