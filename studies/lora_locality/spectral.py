...
DeepSpeed — fits a100-40. Writes {_HERE}/out/<scheme>/spectral/<location>.json
(NOT the main results/ folder).

    python studies/lora_locality/spectral.py --scheme fixedbudget --locations "attn mlp down"
"""
import argparse
import json
import random
...
while _r != _r.parent and not (_r / "src").is_dir():
    _r = _r.parent
sys.path.append(str(_r))

import torch
from transformers import AutoModelForCausalLM, AutoTokenizer

from config import LOCATIONS, SCHEMES, lora_params
...
def main():
    ...
    out_dir = ensure_dir(f"{_HERE}/out/{args.scheme}/spectral")
